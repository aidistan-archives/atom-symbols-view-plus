...
export default class TagGenerator {
  ...
  getProjectRoot() {
    for (let directory of atom.project.getDirectories()) {
      ...
      if (dirPath == this.path || directory.contains(this.dirPath)) {
        return dirPath;
      }
    }
    return null;
  ...
  async generateProjectSymbols() {
    ...
    if (extraCommandArgumentsWhenGeneratingProjectSymbols != '') {
      args.push(extraCommandArgumentsWhenGeneratingProjectSymbols);
    }

    args.push('-o', '.tags', '-R', './');

    return await atom.project.getDirectories().map((directory) => {
      options.cwd = directory.getPath();
      return new Promise((resolve) => new BufferedProcess({
        command: this.command,
        args,
        options,
        stderr: (lines) => atom.notifications.addWarning(
          'Warnings detected during the generation',
          { detail: lines, dismissable: true }
        ),
        exit() { resolve(); },
      }));
    });
