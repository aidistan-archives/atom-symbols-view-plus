...
export default class AutocompleteProvider {
  ...
  watchTagsFiles() {
    ...
    for (let projectPath of atom.project.getPaths()) {
      ...
      if (tagsFilePath) {
        ...
        this.tagsFileSubscriptions.add(tagsFile.onDidChange(this.restartTask.bind(this)));
        this.tagsFileSubscriptions.add(tagsFile.onDidDelete(this.restartTask.bind(this)));
        this.tagsFileSubscriptions.add(tagsFile.onDidRename(this.restartTask.bind(this)));
      }
    }
  }
  ...
  getSuggestions({editor, bufferPosition, scopeDescriptor, prefix, activatedManually}) {
    ...
    return filter(this.tags, prefix, {key: 'name', maxResults: this.maxResults}).map((tag) => {
      ...
      if (scopeDescriptor.scopes[0] === 'text.html.basic') {
